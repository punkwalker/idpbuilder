...
init(){
    export CONFIG_PATH="/root/mock_config.json"
    ...
    echo "Fetching EC2 Mock agent default config..."
    wget https://raw.githubusercontent.com/aws/amazon-ec2-metadata-mock/main/pkg/config/defaults/aemm-metadata-default-values.json -O $CONFIG_PATH
...
refresh_credentials(){
    ...
    echo $(jq --arg session_token "$SESSION_TOKEN" '.metadata.values."iam-security-credentials"["Token"] |= $session_token' $CONFIG_PATH) > $CONFIG_PATH
...
start_mock_loop(){
    ...
    do  
        if [ -z $MOCK_PID ]; then
            ...
            #curl http://169.254.169.254:80/latest/
